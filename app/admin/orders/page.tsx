...
export default async function OrdersAdmin({ searchParams }: { searchParams: Promise<{ updated?: string }> }) {
  ...
  const { data: orders, error } = await supabase.from("orders").select("id,order_number,customer_name,phone,city,total,status,created_at,order_items(quantity,product_name,variant_label)").order("created_at", { ascending: false }).limit(100);
