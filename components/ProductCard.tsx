...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card">
    <div className="product-visual">{product.isNew && <span className="badge">NEW</span>}<button className="wish" aria-label="Add to wishlist"><Heart/></button>{primaryImage ? <Image className="product-photo" src={primaryImage} alt={product.name} fill sizes="(max-width: 560px) 90vw, (max-width: 900px) 45vw, 280px"/> : <PhoneArt color={product.color} small/>}</div>
